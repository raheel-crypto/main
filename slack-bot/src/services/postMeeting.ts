import { appendAudit, insertMeetingRun } from "../db/queries.js";
import { getEvent } from "./googleClient.js";

export interface PostMeetingResult {
  ok: boolean;
  reason?: string;
}

// Stage 2 stub. Replaced by the real SF-update card in stage 4.
export async function runPostMeeting(args: {
  slackUserId: string;
  eventId: string;
}): Promise<PostMeetingResult> {
  const { slackUserId, eventId } = args;
  try {
    const event = await getEvent(slackUserId, eventId);
    console.log(
      `[post-meeting] (stub) ${slackUserId} event=${eventId} title="${event.summary ?? ""}" end=${event.end?.dateTime ?? ""} attendees=${event.attendees?.length ?? 0}`
    );
  } catch (err: any) {
    console.error(`[post-meeting] getEvent failed:`, err?.message ?? err);
  }
  const ins = await insertMeetingRun({
    slackUserId,
    gcalEventId: eventId,
    phase: "post",
  });
  if (!ins.inserted) {
    return { ok: true, reason: "already_ran" };
  }
  await appendAudit({
    slackUserId,
    action: "meeting_post_surfaced",
    metadata: { eventId, stub: true },
  });
  return { ok: true, reason: "stub_logged" };
}

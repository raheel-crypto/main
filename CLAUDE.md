...
- **Per-rep real-time subscriptions** live on a separate surface from standup config: `/subscriptions` (and `/subscriptions_dev` in the dev workspace) opens `subscriptionsModalView` (`slack/subscriptionsModal.ts`). Four input blocks (Gong + Nooks-host + Nooks-firehose + Calendar). Gong has two checkboxes: a per-host "DM me after every Gong call I host" and an admin firehose. Nooks gets **three checkboxes per side** (Positive / Neutral / Negative) — reps and admins each pick which call dispositions they want; only opted-in buckets fire. Calendar has two checkboxes: pre-meeting brief (5-10 min before) and post-meeting SF-update card (5-10 min after). Backed by `users.gong_realtime_enabled`, `users.gong_firehose_enabled`, `users.nooks_host_{positive,neutral,negative}`, `users.nooks_firehose_{positive,neutral,negative}`, `users.calendar_pre_enabled`, `users.calendar_post_enabled`. Legacy `users.nooks_realtime_enabled` / `nooks_firehose_enabled` columns are still updated for read-side back-compat (derived OR of the three per-side dispositions). The submit handler is `registerSubscriptionsSubmit` in `slack/interactivity.ts`; it writes via `updateSubscriptionPrefs`. The Nooks webhook handler (`services/nooksHandler.ts`) parses each call's disposition into a `NooksDispositionBucket = 'positive' | 'neutral' | 'negative'` and matches per-host (`userWantsHostDisposition`) + firehose (`getNooksFirehoseSubscribersFor(bucket)`). If a rep ticks a Calendar box but has no `gc_tokens` row, the ephemeral submit reply includes a Connect Google Calendar link. Adding a feed = one input block in the modal + one or two columns on `users` + one branch in the webhook/cron handler.
...
- **Structured SF write errors + conversational retry**: `sfWriter.applyRecordFields` and `applyBulkRecordFields` parse Salesforce's per-record `errors[]` array into typed `SfApplyError[] = {statusCode, message, fields[]}` and stash on `audit_log.metadata.errors` (JSONB). The bulk resolved card (`bulkRecordCardResolved` in `slack/blocks.ts`) renders the `fields[]` array inline under each failed row, plus a "*Likely fix:*" block when 2+ records share the same `(statusCode, fields)` signature — the hint includes the exact reply template (`set <FieldName> = "..."`). The single-record `writeAndReplyRecord` thread reply also surfaces the field hints. For the retry loop, a new agent tool `sf_get_recent_failed_writes` queries `getRecentBulkFailures(slackUserId, withinMinutes)` (`db/queries.ts`) which scans `audit_log` for `bulk_record_apply_failed` rows in the last N minutes (default 30), groups by `metadata.batchId`, joins to the `pending_cards` row for the original `BulkRecordUpdateProposal` (recap + fields + record names), and returns up to 3 most-recent failed batches with `failedRecords[], originalFields[], missingFieldsSummary`. The QA prompt instructs the agent: when the rep's follow-up looks like a retry ("set X to Y", "with X = Y", "fix it", or a bare value after merlin flagged a missing field), call `sf_get_recent_failed_writes` first, map the rep's value(s) to the missing field(s), then call `sf_propose_bulk_record_update` with **only the failed records** + the original fields + the new field(s) the rep specified. Validation rules (`FIELD_CUSTOM_VALIDATION_EXCEPTION`) usually have empty `fields[]` so reps just see the admin-authored Error Message; layout-required fields (`REQUIRED_FIELD_MISSING`) and type errors (`INVALID_FIELD_FOR_INSERT_UPDATE`, `STRING_TOO_LONG`, etc.) come with populated `fields[]`. No infrastructure changes; the `errors` JSONB column already accepts arbitrary objects via the existing `metadata` field. Salesforce's data API has no validate-only / dry-run mode — neither REST nor the Hosted MCP exposes one — so the safest pre-flight test is to run a bulk apply against a small sample in a sandbox.
...
**Red Team agent integration** — Merlin acts as the broker between Salesforce/Gong and a separate Python autonomous-agent service (`red-team-agent/`, FastAPI, deployed as its own Vercel project — same monorepo, separate project — sharing the slack-bot's `POSTGRES_URL` for cooldown state via the `red_team_cooldowns` table; entry point is `red-team-agent/api/index.py` re-exporting the FastAPI app, routed by `red-team-agent/vercel.json`) that role-plays adversaries to challenge advanced-stage deals. Merlin owns triggers, intel-pack assembly, the HTTP call, Slack UX, and audit; the Python service stays a stateless evaluation function. Triggers (v1): (a) every Gong webhook on an opp whose `StageName` is in `RED_TEAM_STAGE_ALLOWLIST` — wired into `gongWebhookHandler` after the post-call SF-update branch via `services/redTeamGongTrigger.ts:triggerRedTeamForGongCall` (resolves call→opp from the Gong `context.salesforce` block, or via the Gong-SF Task sync's `Gong__Gong_Call_Id__c`); (b) `/api/red-team/sweep` (Vercel cron, daily 14:00 UTC) enumerates `red_team_enabled=true` reps and fans out to `/api/red-team/run-for-user` (300s budget) which queries each rep's open stage-allowlist opps and fires evals at concurrency 3. The intel pack (`services/redTeamIntelPack.ts:buildIntelPack`) bundles the Opportunity + selected MEDDPICC custom fields, recent activities (30d), recent field changes (tries `OpportunityFieldHistory` first, falls back to a per-opp snapshot diff from the new `opp_snapshots` table — every eval refreshes the snapshot regardless), and the last 5 Gong calls **with verbatim transcripts** (`services/gong.ts:getCallTranscript` POSTs to `/v2/calls/transcript`). The pack is POSTed to `${RED_TEAM_AGENT_URL}/evaluate` with an HMAC-SHA256 signature in `X-RedTeam-Signature` (see `services/redTeamClient.ts`). The agent's structured response (`RedTeamRunResult` — Zod-validated) carries `firedTriggers`, up to N `personasInvoked` arguments, each with `headline`, `claim`, `citations[]`, `riskScore`. Merlin renders via `slack/redTeamCard.ts:redTeamCard` and DMs the opp owner (or `RED_TEAM_REDIRECT_TO_SLACK_USER_ID` during the manager-only rollout). Card has *Open in Salesforce* + *Mute this opp for 7 days* (writes `red_team_mutes(opportunity_id, slack_user_id, muted_until)`). Opt-in lives in `/subscriptions` (new "Red Team" checkbox → `users.red_team_enabled`). `RED_TEAM_SHADOW_MODE=true` (default) skips the Slack post but still POSTs to the agent and writes `red_team_eval_shadow` audit rows for week-1 review. Audit actions: `red_team_intel_surfaced` / `red_team_intel_dropped` (with `reason` ∈ `agent_not_configured | sf_not_connected | stage_not_eligible | muted | no_opp_link | no_personas_fired | user_muted | …`) / `red_team_intel_failed` / `red_team_eval_shadow`. Probes: `npm run probe -w slack-bot red-team-pack <slack_user_id> <opp_id>` prints the assembled pack JSON without calling the agent; `red-team <slack_user_id> <opp_id>` runs the full eval; `red-team-sweep <slack_user_id>` runs the daily sweep step for one rep. Phase 1 is Red Team only; Blue Team + Arbiter will follow the same broker pattern (separate Python services, Merlin renders combined output).

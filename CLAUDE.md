...
- **Opportunity updates from DM**: one write-staging tool `sf_propose_opportunity_update` lets the rep say `@merlin update Acme Q4 — NextStep: ship MSA, CloseDate: today`. The tool validates fields against the writable allowlist (`StageName, NextStep, Amount, CloseDate, Notes__c, Deal_Description__c` — Rogo's org barely uses the standard `Description` field, so the allowlist routes long-text writes to the two custom 32K-char fields instead: `Notes__c` for free-form notes ("notes", "call notes") and `Deal_Description__c` for the structured deal write-up ("description", "deal description", "overview")). The tool checks the StageName picklist, looks up current values, and stages the proposal on `AgentToolCtx.pendingWriteProposal`. After the agent loop returns, `services/qa.ts` inserts `pending_cards.kind='qa_proposal'` and posts the standard `oppCard` into the DM — the SF write only fires when the rep clicks Accept/Edit/Apply-all, going through the same `sfWriter.applyFields` path as the standup. The five `card.kind !== "standup"` narrowing checks in `slack/interactivity.ts` now also accept `qa_proposal`. Audit action: `qa_proposed_update` (per field, at proposal time) → existing `accepted`/`skipped`/`edited`/`applied` (at button click).

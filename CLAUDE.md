...
- **Structured SF write errors**: `sfWriter.applyRecordFields` and `applyBulkRecordFields` parse Salesforce's per-record `errors[]` array into typed `SfApplyError[] = {statusCode, message, fields[]}` and stash on `audit_log.metadata.errors` (JSONB) — used to be the joined human message only. The bulk resolved card (`bulkRecordCardResolved` in `slack/blocks.ts`) renders the `fields[]` array inline under each failed row, plus a "*Likely fix:*" summary block when 2+ records share the same `(statusCode, fields)` signature ("3 records failed with REQUIRED_FIELD_MISSING — need `Closed_Lost_Reason__c`. Reply with values to set and I'll retry just the failures."). The single-record `writeAndReplyRecord` thread reply also surfaces the field hints. Validation rules (`FIELD_CUSTOM_VALIDATION_EXCEPTION`) usually have empty `fields[]` so reps just see the admin-authored Error Message; layout-required fields (`REQUIRED_FIELD_MISSING`) and type errors (`INVALID_FIELD_FOR_INSERT_UPDATE`, `STRING_TOO_LONG`, etc.) come with populated `fields[]`. No infrastructure changes; the `errors` JSONB column already accepts arbitrary objects via the existing `metadata` field. Salesforce's data API has no validate-only / dry-run mode — neither REST nor the Hosted MCP exposes one — so the safest pre-flight test is to run a bulk apply against a small sample in a sandbox.

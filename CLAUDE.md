...
- **Per-rep real-time subscriptions** live on a separate surface from standup config: `/subscriptions` (and `/subscriptions_dev` in the dev workspace) opens `subscriptionsModalView` (`slack/subscriptionsModal.ts`) with a checkbox per real-time feed. v1 ships one checkbox ("DM me a digest after every Gong call I host") backed by `users.gong_realtime_enabled`. The submit handler is `registerSubscriptionsSubmit` in `slack/interactivity.ts`; it writes via `updateSubscriptionPrefs` so it doesn't touch the standup columns. Adding a feed = one row in the modal + one column on `users` + one branch in the webhook handler.
